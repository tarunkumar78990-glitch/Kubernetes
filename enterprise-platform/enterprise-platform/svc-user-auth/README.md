    ...
    **Runbook:** `platform-ops/runbooks/user-auth.md`
    ...
    **Called by:** `frontend`
    ...
    | Method | Path | Purpose |
|---|---|---|
| POST | `/api/auth/register` | Create account (PBKDF2 hashing) |
| POST | `/api/auth/login` | Get a JWT |
| GET | `/api/auth/verify` | Verify `Authorization: Bearer <token>` |
...
```
    ...
    ```bash
    docker build -t svc-user-auth:local .
    docker run -p 8080:8080 svc-user-auth:local
    ...
    ```bash
    export IMAGE_URL="asia-south1-docker.pkg.dev/PROJECT/dev-microservices/svc-user-auth:sometag"
...
|---|---|---|
| `JWT_SECRET` | GCP Secret Manager | See `README-SECRETS.md` |
