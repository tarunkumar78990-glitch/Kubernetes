    ...
    Shopping carts. Validates products and snapshots prices at add time.

    **Stack:** Node.js 20 / Express
    **Tier:** 2 · **Availability SLO:** 99.9% over 30 days
    **Runbook:** `platform-ops/runbooks/cart.md`
    ...
    **Called by:** `checkout`, `frontend`
    ...
    | Method | Path | Purpose |
|---|---|---|
| GET | `/api/carts/:userId` | Get cart with total |
| POST | `/api/carts/:userId/items` | Add item (validates via catalog) |
...
```
    ...
    ```bash
    docker build -t svc-cart:local .
    docker run -p 8080:8080 svc-cart:local
    ...
    ```bash
    export IMAGE_URL="asia-south1-docker.pkg.dev/PROJECT/dev-microservices/svc-cart:sometag"
