    ...
    Transactional messages. Best-effort by design.

    **Stack:** Python 3.12 / FastAPI
    **Tier:** 3 · **Availability SLO:** 99.0% over 30 days
    **Runbook:** `platform-ops/runbooks/notification.md`
    ...
    **Called by:** `checkout`
    ...
    | Method | Path | Purpose |
|---|---|---|
| POST | `/api/notifications/send` | Queue a message (202 Accepted) |
...
```
    ...
    ```bash
    docker build -t svc-notification:local .
    docker run -p 8080:8080 svc-notification:local
    ...
    ```bash
    export IMAGE_URL="asia-south1-docker.pkg.dev/PROJECT/dev-microservices/svc-notification:sometag"
...
|---|---|---|
| `SMTP_API_KEY` | GCP Secret Manager | See `README-SECRETS.md` |
