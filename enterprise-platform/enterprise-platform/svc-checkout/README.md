    ...
    Orchestrates the purchase. The fan-out lives here.

    **Stack:** Node.js 20 / Express
    **Tier:** 1 · **Availability SLO:** 99.95% over 30 days
    **Runbook:** `platform-ops/runbooks/checkout.md`

    ---

    ## Where it sits

    **Calls:** `svc-cart`, `svc-payment`, `svc-shipping`, `svc-order`, `svc-notification`
    **Called by:** `frontend`
    ...
    | Method | Path | Purpose |
|---|---|---|
| POST | `/api/checkout` | Run the full purchase flow |
...
```
    ...
    ```bash
    docker build -t svc-checkout:local .
    docker run -p 8080:8080 svc-checkout:local
    ...
    ```bash
    export IMAGE_URL="asia-south1-docker.pkg.dev/PROJECT/dev-microservices/svc-checkout:sometag"
