    ...
    Backend-for-frontend. Aggregates several services into one call for the browser.

    **Stack:** Node.js 20 / Express
    **Tier:** 1 · **Availability SLO:** 99.9% over 30 days
    **Runbook:** `platform-ops/runbooks/frontend.md`

    ---

    ## Where it sits

    **Calls:** `svc-product-catalog`, `svc-cart`, `svc-order`, `svc-recommendation`, `svc-checkout`
    **Called by:** `public internet`

    ## API

    | Method | Path | Purpose |
|---|---|---|
| GET | `/api/home?userId=` | Catalog + recommendations, aggregated |
| GET | `/api/dashboard/:userId` | Cart + order history |
| POST | `/api/checkout` | Proxy to the checkout service |
...
```
    ...
    ```bash
    docker build -t svc-frontend:local .
    docker run -p 8080:8080 svc-frontend:local
    ...
    ```bash
    export IMAGE_URL="asia-south1-docker.pkg.dev/PROJECT/dev-microservices/svc-frontend:sometag"
