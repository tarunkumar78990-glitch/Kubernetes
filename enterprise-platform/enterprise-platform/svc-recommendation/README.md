    ...
    Product suggestions. Degrades gracefully if the catalog is down.

    **Stack:** Python 3.12 / FastAPI
    **Tier:** 3 · **Availability SLO:** 99.0% over 30 days
    **Runbook:** `platform-ops/runbooks/recommendation.md`

    ---

    ## Where it sits

    **Calls:** `svc-product-catalog`
    **Called by:** `frontend`
    ...
    | Method | Path | Purpose |
|---|---|---|
| GET | `/api/recommendations/{userId}` | Suggestions (degrades to `[]`) |
| POST | `/api/recommendations/{userId}/viewed` | Record a product view |
...
```
    ...
    ```bash
    docker build -t svc-recommendation:local .
    docker run -p 8080:8080 svc-recommendation:local
    ...
    ```bash
    export IMAGE_URL="asia-south1-docker.pkg.dev/PROJECT/dev-microservices/svc-recommendation:sometag"
