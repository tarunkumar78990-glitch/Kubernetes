    ...
    **Runbook:** `platform-ops/runbooks/product-catalog.md`
...
| POST | `/api/products/:id/reserve` | Reserve stock (409 if insufficient) |
...
```
    ...
    ```bash
    docker build -t svc-product-catalog:local .
    docker run -p 8080:8080 svc-product-catalog:local
    ...
    ```bash
    export IMAGE_URL="asia-south1-docker.pkg.dev/PROJECT/dev-microservices/svc-product-catalog:sometag"
