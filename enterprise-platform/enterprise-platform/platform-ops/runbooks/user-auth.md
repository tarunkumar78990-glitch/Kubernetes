...
Registration, login and token verification. Holds the JWT signing secret.
...
## Alert: UserAuthErrorBudgetFastBurn
...
kubectl get pods -n prod -l app=user-auth -o wide

# 2. What changed? Almost always the answer.
kubectl rollout history deployment/user-auth -n prod
kubectl get deployment user-auth -n prod \
  -o jsonpath='{.metadata.annotations.platform/git-commit}'

# 3. Actual errors, not guesses
kubectl logs -n prod -l app=user-auth --tail=100 | grep -i error
```
...
```bash
kubectl rollout undo deployment/user-auth -n prod
kubectl rollout status deployment/user-auth -n prod
```
...
```bash
kubectl top pods -n prod -l app=user-auth
kubectl describe pod -n prod -l app=user-auth | grep -A5 "Last State"
```
...
## Alert: UserAuthErrorBudgetSlowBurn
...
## Alert: UserAuthLatencySLOBreach

```bash
# Where is time going? Check the dependency call rate first.
# In Prometheus:
#   histogram_quantile(0.99,
#     sum by (le) (rate(http_request_duration_seconds_bucket{app="user-auth"}[5m])))
```
...
## Alert: UserAuthErrorBudgetExhausted
...
slo:user_auth:error_budget_remaining

# Error rate right now
1 - slo:user_auth:success_ratio5m

# Which downstream is failing?
sum by (dependency, status) (
  rate(dependency_requests_total{app="user-auth"}[5m])
)

# Request rate by status
sum by (status_code) (rate(http_requests_total{app="user-auth"}[5m]))
```
...
- Dashboard: Grafana → `user-auth overview`
- Repo: `svc-user-auth`
