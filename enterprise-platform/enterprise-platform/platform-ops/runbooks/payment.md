...
Authorises, captures and refunds payments. Holds the payment gateway key (from Secret Manager via Workload Identity).
...
kubectl logs -n prod -l app=payment --tail=100 | grep -i error
```
...
```bash
kubectl top pods -n prod -l app=payment
kubectl describe pod -n prod -l app=payment | grep -A5 "Last State"
```
...
#     sum by (le) (rate(http_request_duration_seconds_bucket{app="payment"}[5m])))
```
...
## Alert: PaymentErrorBudgetExhausted
...
1 - slo:payment:success_ratio5m

# Which downstream is failing?
sum by (dependency, status) (
  rate(dependency_requests_total{app="payment"}[5m])
)

# Request rate by status
sum by (status_code) (rate(http_requests_total{app="payment"}[5m]))
```
...
- Dashboard: Grafana → `payment overview`
- Repo: `svc-payment`
