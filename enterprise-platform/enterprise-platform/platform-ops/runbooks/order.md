...
Creates and stores orders. Reserves stock in the catalog before confirming.
...
## Alert: OrderErrorBudgetFastBurn
...
kubectl get pods -n prod -l app=order -o wide

# 2. What changed? Almost always the answer.
kubectl rollout history deployment/order -n prod
kubectl get deployment order -n prod \
  -o jsonpath='{.metadata.annotations.platform/git-commit}'

# 3. Actual errors, not guesses
kubectl logs -n prod -l app=order --tail=100 | grep -i error
```
...
```bash
kubectl rollout undo deployment/order -n prod
kubectl rollout status deployment/order -n prod
```
...
```bash
kubectl top pods -n prod -l app=order
kubectl describe pod -n prod -l app=order | grep -A5 "Last State"
```
...
## Alert: OrderErrorBudgetSlowBurn
...
## Alert: OrderLatencySLOBreach

```bash
# Where is time going? Check the dependency call rate first.
# In Prometheus:
#   histogram_quantile(0.99,
#     sum by (le) (rate(http_request_duration_seconds_bucket{app="order"}[5m])))
```
...
slo:order:error_budget_remaining

# Error rate right now
1 - slo:order:success_ratio5m

# Which downstream is failing?
sum by (dependency, status) (
  rate(dependency_requests_total{app="order"}[5m])
)

# Request rate by status
sum by (status_code) (rate(http_requests_total{app="order"}[5m]))
```
...
- Dashboard: Grafana → `order overview`
- Repo: `svc-order`
