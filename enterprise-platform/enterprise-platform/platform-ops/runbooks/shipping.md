...
**Latency SLO:** 500ms
**Error budget:** 216.0 minutes of full downtime per 30 days

---

## What this service does

Calculates shipping cost and delivery estimates by zone.
...
## Alert: ShippingErrorBudgetFastBurn
...
kubectl get pods -n prod -l app=shipping -o wide

# 2. What changed? Almost always the answer.
kubectl rollout history deployment/shipping -n prod
kubectl get deployment shipping -n prod \
  -o jsonpath='{.metadata.annotations.platform/git-commit}'

# 3. Actual errors, not guesses
kubectl logs -n prod -l app=shipping --tail=100 | grep -i error
```
...
```bash
kubectl rollout undo deployment/shipping -n prod
kubectl rollout status deployment/shipping -n prod
```
...
```bash
kubectl top pods -n prod -l app=shipping
kubectl describe pod -n prod -l app=shipping | grep -A5 "Last State"
```
...
## Alert: ShippingErrorBudgetSlowBurn
...
## Alert: ShippingLatencySLOBreach

```bash
# Where is time going? Check the dependency call rate first.
# In Prometheus:
#   histogram_quantile(0.99,
#     sum by (le) (rate(http_request_duration_seconds_bucket{app="shipping"}[5m])))
```
...
## Alert: ShippingErrorBudgetExhausted
...
slo:shipping:error_budget_remaining

# Error rate right now
1 - slo:shipping:success_ratio5m

# Which downstream is failing?
sum by (dependency, status) (
  rate(dependency_requests_total{app="shipping"}[5m])
)

# Request rate by status
sum by (status_code) (rate(http_requests_total{app="shipping"}[5m]))
```
...
- Dashboard: Grafana → `shipping overview`
- Repo: `svc-shipping`
