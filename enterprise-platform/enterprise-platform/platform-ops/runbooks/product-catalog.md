...
Source of truth for products, prices and stock. Nearly everything depends on it — treat its SLO as load-bearing.

**Depends on:** none
**Called by:** cart, order, recommendation, frontend

---

## Alert: ProductCatalogErrorBudgetFastBurn
...
kubectl get pods -n prod -l app=product-catalog -o wide

# 2. What changed? Almost always the answer.
kubectl rollout history deployment/product-catalog -n prod
kubectl get deployment product-catalog -n prod \
  -o jsonpath='{.metadata.annotations.platform/git-commit}'

# 3. Actual errors, not guesses
kubectl logs -n prod -l app=product-catalog --tail=100 | grep -i error
```
...
```bash
kubectl rollout undo deployment/product-catalog -n prod
kubectl rollout status deployment/product-catalog -n prod
```

### If no deploy went out

Check the dependencies. This service calls: none
...
```bash
kubectl top pods -n prod -l app=product-catalog
kubectl describe pod -n prod -l app=product-catalog | grep -A5 "Last State"
```
...
## Alert: ProductCatalogErrorBudgetSlowBurn
...
## Alert: ProductCatalogLatencySLOBreach

```bash
# Where is time going? Check the dependency call rate first.
# In Prometheus:
#   histogram_quantile(0.99,
#     sum by (le) (rate(http_request_duration_seconds_bucket{app="product-catalog"}[5m])))
```
...
## Alert: ProductCatalogErrorBudgetExhausted
...
slo:product_catalog:error_budget_remaining

# Error rate right now
1 - slo:product_catalog:success_ratio5m

# Which downstream is failing?
sum by (dependency, status) (
  rate(dependency_requests_total{app="product-catalog"}[5m])
)

# Request rate by status
sum by (status_code) (rate(http_requests_total{app="product-catalog"}[5m]))
```
...
- Dashboard: Grafana → `product-catalog overview`
- Repo: `svc-product-catalog`
