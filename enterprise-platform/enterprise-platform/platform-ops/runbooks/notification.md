...
Sends order confirmations and other transactional messages. Deliberately best-effort: checkout does NOT fail if this fails.
...
## Alert: NotificationErrorBudgetFastBurn
...
kubectl get pods -n prod -l app=notification -o wide

# 2. What changed? Almost always the answer.
kubectl rollout history deployment/notification -n prod
kubectl get deployment notification -n prod \
  -o jsonpath='{.metadata.annotations.platform/git-commit}'

# 3. Actual errors, not guesses
kubectl logs -n prod -l app=notification --tail=100 | grep -i error
```
...
```bash
kubectl rollout undo deployment/notification -n prod
kubectl rollout status deployment/notification -n prod
```
...
```bash
kubectl top pods -n prod -l app=notification
kubectl describe pod -n prod -l app=notification | grep -A5 "Last State"
```
...
## Alert: NotificationErrorBudgetSlowBurn
...
## Alert: NotificationLatencySLOBreach

```bash
# Where is time going? Check the dependency call rate first.
# In Prometheus:
#   histogram_quantile(0.99,
#     sum by (le) (rate(http_request_duration_seconds_bucket{app="notification"}[5m])))
```
...
## Alert: NotificationErrorBudgetExhausted
...
slo:notification:error_budget_remaining

# Error rate right now
1 - slo:notification:success_ratio5m

# Which downstream is failing?
sum by (dependency, status) (
  rate(dependency_requests_total{app="notification"}[5m])
)

# Request rate by status
sum by (status_code) (rate(http_requests_total{app="notification"}[5m]))
```
...
- Dashboard: Grafana → `notification overview`
