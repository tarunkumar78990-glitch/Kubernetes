    ...
    Authorise, capture and refund payments.

    **Stack:** Python 3.12 / FastAPI
    **Tier:** 1 · **Availability SLO:** 99.95% over 30 days
    **Runbook:** `platform-ops/runbooks/payment.md`
    ...
    **Called by:** `checkout`
    ...
    | Method | Path | Purpose |
|---|---|---|
| POST | `/api/payments/authorize` | Authorise (~3% simulated decline) |
| POST | `/api/payments/{id}/capture` | Capture an authorised payment |
...
```
    ...
    ```bash
    docker build -t svc-payment:local .
    docker run -p 8080:8080 svc-payment:local
    ...
    ```bash
    export IMAGE_URL="asia-south1-docker.pkg.dev/PROJECT/dev-microservices/svc-payment:sometag"
...
This service reads secrets. It uses **Workload Identity** — there is no
JSON key anywhere in the image or on disk.

| Variable | Source | Notes |
|---|---|---|
| `PAYMENT_GATEWAY_KEY` | GCP Secret Manager | See `README-SECRETS.md` |

Locally, a fallback dev value is used so you can run without GCP.
