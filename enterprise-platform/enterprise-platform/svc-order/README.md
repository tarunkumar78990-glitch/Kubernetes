    ...
    Creates and stores orders. Reserves stock before confirming.

    **Stack:** Node.js 20 / Express
    **Tier:** 1 · **Availability SLO:** 99.9% over 30 days
    **Runbook:** `platform-ops/runbooks/order.md`
...
```
    ...
    ```bash
    docker build -t svc-order:local .
    docker run -p 8080:8080 svc-order:local
    ...
    ```bash
    export IMAGE_URL="asia-south1-docker.pkg.dev/PROJECT/dev-microservices/svc-order:sometag"
